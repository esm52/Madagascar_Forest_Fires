...
```{r}
CassavaPlot <- ggplot(FullData, aes(x = CassavaArea, y = CassavaYield, color= Year)) + 
  geom_point() + theme(legend.position="none") +
  geom_path()
CassavaPlot

ClovesPlot <- ggplot(FullData, aes(x = ClovesArea, y = ClovesYield, color= Year)) + 
  geom_point() + theme(legend.position="none") +
  geom_path()
ClovesPlot

CocoaPlot <- ggplot(FullData, aes(x = CocoaArea, y = CocoaYield, color= Year)) + 
  geom_point() + theme(legend.position="none") +
  geom_path()
CocoaPlot

CoffeePlot <- ggplot(FullData, aes(x = CoffeeArea, y = CoffeeYield, color= Year)) + 
  geom_point() + theme(legend.position="none") +
  geom_path()
CoffeePlot #very clear example of increased agricultural intensity over time less area needed to create higher yield

MaizePlot <- ggplot(FullData, aes(x = MaizeArea, y = MaizeYield, color= Year)) + 
  geom_point() + theme(legend.position="none") +
  geom_path()
MaizePlot

RicePlot <- ggplot(FullData, aes(x = RiceArea, y = RiceYield, color= Year)) + 
  geom_point() + theme(legend.position="none") +
  geom_path()
RicePlot #really interesting pattern of increased agricultural intensity - showing learning curve over time 

SweetPotatoPlot <- ggplot(FullData, aes(x = SweetPotatoArea, y = SweetPotatoYield, color= Year)) + 
  geom_point() + theme(legend.position="none") +
  geom_path()
SweetPotatoPlot #shows opposite trend - relationship between area and yield appears to remain constant throughout years

VanillaPlot <- ggplot(FullData, aes(x = VanillaArea, y = VanillaYield, color= Year)) + 
  geom_point() + theme(legend.position="none") +
  geom_path()
